...
//Top Module Code Starts here:
	module top(input logic clk, input logic reset, input logic [31:0] cyc_cnt, output logic passed, output logic failed);
		logic  rst;//input
		logic  [17:0] instruction_in;//input
		logic  [15:0] data_in;//input
		logic  [15:0] data_out;//output
		logic  [9:0] instruction_addr;//output
		logic  [9:0] data_addr;//output
      logic  [17:0] MEMA [256];
      logic  [15:0] DATAM [256];
		logic  data_R;//output
		logic  data_W;//output
		logic  done;//output
      integer i=0;
//The $random() can be replaced if user wants to assign values
		//assign rst = 0;
      assign MEMA [0] = 18'b100110000000000000;
      assign MEMA [1] = 18'b100110010000000001;
      assign MEMA [2] = 18'b100110100000000010;
      assign MEMA [3] = 18'b000000000000000001;
      assign MEMA [4]  = 18'b000000000000001000; // add H0 to H2               add16bit R2=0000000000000100 R0=0000000000010100
  assign MEMA [5]  = 18'b101001_00_0000000000;  // store H0 back into MEM0    store16bit R0=0000000000010100 im=0000000000
  assign MEMA [6]  = 18'b101100_01_0100100010;  // set H1 to 0100100010       set16bit R1 im=0100100010
  assign MEMA [7]  = 18'b000000_00000000_01_10; // add H2 to H1               add16bit R1=0000000100100010 R2=0000000000011000
  assign MEMA [8]  = 18'b101001_01_0000000011;  // store H1 back into MEM3    store16bit R1=0000000100111010 im=0000000011
  assign MEMA [9]  = 18'b101001_10_0000000100;  // store H2 back into MEM4    store16bit R2=0000000000011000 im=0000000100
  assign MEMA [10] = 18'b101001_00_0000000101; 
      assign DATAM [0] = 1;
      assign DATAM [1] = 2;
      assign DATAM [2] = 3;
    ...
    begin
        if (data_W) 
	begin
		DATAM[data_addr] <= data_out;
	end
        else
            data_in <= DATAM[data_addr];
      ...
      always @(negedge clk) begin
         if ((cyc_cnt>3)) begin
				instruction_in <= MEMA[instruction_addr];
                     i=i+1;
         end
         //if (i < 8) begin
			//	assign data_in = DATAM[i-5];
         //end

         end
		CPUtop CPUtop(.clk(clk), .rst(rst), .instruction_in(instruction_in), .data_in(data_in), .data_out(data_out), .instruction_addr(instruction_addr), .data_addr(data_addr), .data_R(data_R), .data_W(data_W), .done(done));
