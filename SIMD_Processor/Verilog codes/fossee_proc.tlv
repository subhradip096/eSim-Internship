...
//The $random() can be replaced if user wants to assign values
		...
		assign b = 2;
		assign c = 3;
		fossee_proc fossee_proc(.clk(clk), .rst(rst), .a(a), .b(b), .c(c), .done(done), .instruction_in(instruction_in), .data_in(data_in), .data_out(data_out), .instruction_addr(instruction_addr), .data_addr(data_addr), .data_R(data_R), .data_W(data_W));
